...
%    y = NLDIF(..., 'eedif',{p, lambda, sigma, rho, m, stepsize, steps}) performs an edge enhancing diffusion 
%         after every p steps using the given parameters. To use in the eedif the same parameters used in the
...
for i=1:nosteps
   ...
   if eed ~= 0
      eestep = ( mod(i-1,eedpar{1})==0 & i~=1 ); % Current step will use edge enhancing diffusion.
   else
      eestep = 0;
   end
